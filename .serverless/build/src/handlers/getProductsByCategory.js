...
// src/handlers/getProductsByCategory.ts
var getProductsByCategory_exports = {};
__export(getProductsByCategory_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(getProductsByCategory_exports);
...
var handler = async (event) => {
  try {
    ...
    if (searchLower) {
      const params = {
        TableName: "Products",
        FilterExpression: "isActive = :active AND contains(#st, :q) AND #cid = :cid AND #qty >= :minQty",
      ...
      const res = await ddb.send(new import_lib_dynamodb3.ScanCommand(params));
