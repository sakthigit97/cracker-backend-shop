...
var orderInvoice_exports = {};
__export(orderInvoice_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(orderInvoice_exports);
...
async function handler(event) {
  try {
    ...
    try {
      await s3.send(
        new import_client_s32.HeadObjectCommand({
          Bucket: BUCKET,
          Key: key
        })
      );
    ...
    const command = new import_client_s32.GetObjectCommand({
      ...
      ResponseContentDisposition: `attachment; filename=bill-${orderId}.pdf`
    });
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({
        message: error?.message || "Unable to retrieve bill."
      })
    };
  }
}
