...
// src/repo/cart.repo.ts
var TABLE_NAME = process.env.CART_TABLE;
var CartRepository = class {
  ...
  async setItemQuantity(cartId, productId, qty) {
    await ddb.send(
...
var CartService = class {
  ...
  async addItem(pk, productId, qty) {
    if (qty === 0) return;
    await this.repo.addItem(pk, productId, qty);
  }
  async removeItem(pk, productId) {
...
function resolveCartContext(event) {
  ...
  let guestId = event.headers?.["x-guest-id"] || event.headers?.["X-Guest-Id"];
  if (!guestId) {
    guestId = import_crypto.default.randomUUID();
  }
  return {
    pk: `GUEST#${guestId}`,
    guestId
