...
// src/handlers/getRevenueReport.ts
var getRevenueReport_exports = {};
__export(getRevenueReport_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(getRevenueReport_exports);
...
var RevenueService = class {
  async getRevenueReport(params) {
    ...
    if (fromDate && toDate) {
      ...
      currentFromTime = this.startOfDay(from).getTime();
      currentToTime = this.endOfDay(to).getTime();
    } else {
      const days = this.getDays(range || "7d");
      const today = /* @__PURE__ */ new Date();
      const todayStart = this.startOfDay(today);
      const currentFrom = new Date(todayStart);
      currentFrom.setDate(
        currentFrom.getDate() - (days - 1)
      );
      currentFromTime = currentFrom.getTime();
      currentToTime = this.endOfDay(today).getTime();
    }
    const currentStartDate = new Date(currentFromTime);
    const currentEndDate = new Date(currentToTime);
    const periodDays = this.getInclusiveDayCount(
      currentStartDate,
      currentEndDate
    );
    ...
    previousStartDate.setDate(
      previousStartDate.getDate() - (periodDays - 1)
    );
    const previousFromTime = this.startOfDay(
      previousStartDate
    ).getTime();
    const previousToTime = this.endOfDay(
      previousEndDate
    ).getTime();
    ...
    do {
      ...
      for (const item of items) {
        ...
        const amount = Number(
          item.finalPayable?.N || 0
        ) > 0 ? Number(
          item.finalPayable?.N || 0
        ) : Number(
          item.grandTotal?.N || 0
        );
        if (createdAt >= currentFromTime && createdAt <= currentToTime) {
          totalRevenue += amount;
          totalOrders += 1;
          const date = new Date(
            createdAt
        ...
        if (createdAt >= previousFromTime && createdAt <= previousToTime) {
    ...
    return {
      ...
      yesterdayRevenue: previousRevenue,
      growth,
      trend
    };
  }
  ...
  getInclusiveDayCount(from, to) {
    const fromDay = this.startOfDay(from);
    const toDay = this.startOfDay(to);
    return Math.floor(
      (toDay.getTime() - fromDay.getTime()) / DAY_MS
    ) + 1;
  }
};
...
*/
//# sourceMappingURL=getRevenueReport.js.map
