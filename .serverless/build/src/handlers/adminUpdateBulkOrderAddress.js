...
// src/handlers/adminUpdateBulkOrderAddress.ts
var adminUpdateBulkOrderAddress_exports = {};
__export(adminUpdateBulkOrderAddress_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(adminUpdateBulkOrderAddress_exports);
...
var BulkOrderRepository = class {
  ...
  async updateStatus(orderId, data) {
    ...
    if (data.status === "PAYMENT_CONFIRMED") {
      ...
      expressionAttributeValues[":paymentAccountId"] = data.paymentAccountId.trim();
    }
    await ddb.send(
      new import_lib_dynamodb2.UpdateCommand({
        TableName: TABLE_NAME,
...
var AdminCodeRepository = class {
  ...
  static async listByUserId(userId) {
    ...
    const result = await ddb.send(
      new import_lib_dynamodb6.ScanCommand({
        TableName: TABLE,
        FilterExpression: "#userId = :userId AND #status = :status AND #expiryDate > :now",
        ExpressionAttributeNames: {
          "#userId": "userId",
          "#status": "status",
...
var AdminCodeService = class {
  ...
  static async listCodesForUser(userId) {
    const codes = await AdminCodeRepository.listByUserId(
      userId
    );
    return codes.sort(
      (a, b) => b.createdAt - a.createdAt
...
var BulkOrderService = class {
  ...
  getScheme(schemeId, config) {
    const schemes = config?.bulkOrderSchemes;
  ...
  calculateSchemePrice(product, scheme) {
    ...
    const adjustmentPercent = Number(
      scheme?.bulkPriceAdjustmentPercent ?? 0
    );
    if (!adjustmentPercent || adjustmentPercent <= 0) {
      return basePrice;
    }
    const adjustmentAmount = basePrice * adjustmentPercent / 100;
    if (scheme.bulkPriceAdjustmentType === "MINUS") {
      return Math.max(
        0,
        Math.round(
    ...
    if (scheme.bulkPriceAdjustmentType === "PLUS") {
      return Math.round(
        basePrice + adjustmentAmount
      );
    }
  ...
  buildItems(request, productMap, scheme) {
  ...
  validateScheme(scheme, productTotal) {
  ...
  generateOrderId(now) {
    ...
    const ymd = d.getFullYear().toString() + String(
      d.getMonth() + 1
    ).padStart(2, "0") + String(
      d.getDate()
    ).padStart(2, "0");
  ...
  addStatusHistory(history = [], status, by, comment) {
    return [
      ...history,
      {
        status,
  ...
  async updateStatus(orderId, status, adminId, comment, paymentAccountId) {
    const order = await this.repo.getById(
      orderId
    );
    if (!order) {
    ...
    if (status === "PAYMENT_CONFIRMED" && !paymentAccountId?.trim()) {
      throw new Error(
        "Payment account is required when confirming payment."
      );
    }
  ...
  async updateAddress(orderId, address, adminId) {
    const order = await this.repo.getById(orderId);
    if (!order) {
...
// src/handlers/adminUpdateBulkOrderAddress.ts
var service = new BulkOrderService();
var handler = async (event) => {
  try {
    ...
    if (message === "Bulk order not found.") {
      return {
        statusCode: 404,
        body: JSON.stringify({ message })
      };
    }
    if (message.includes("cannot be updated") || message.includes("required")) {
      return {
        statusCode: 400,
        body: JSON.stringify({ message })
      };
...
*/
//# sourceMappingURL=adminUpdateBulkOrderAddress.js.map
