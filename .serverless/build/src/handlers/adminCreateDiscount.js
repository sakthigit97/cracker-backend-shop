...
var adminCreateDiscount_exports = {};
__export(adminCreateDiscount_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(adminCreateDiscount_exports);
...
var handler = async (event) => {
  try {
    ...
    const isApplyToAll = body.discountType === "PRODUCT" && body.applyToAll === true;
    if (!isApplyToAll && !body.targetId) {
      return {
        statusCode: 400,
    ...
    if (isApplyToAll) {
      const created2 = await service.createDiscountForAllProducts(
        body
      );
      return {
        statusCode: 200,
        body: JSON.stringify(created2)
      };
    }
    const exists = await service.existsByTargetId(
      body.targetId
    );
    if (exists) {
      return error(
...
*/
//# sourceMappingURL=adminCreateDiscount.js.map
