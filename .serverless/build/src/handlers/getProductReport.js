...
// src/handlers/getProductReport.ts
var getProductReport_exports = {};
__export(getProductReport_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(getProductReport_exports);
...
// src/services/productReport.service.ts
var import_client_dynamodb = require("@aws-sdk/client-dynamodb");
var client = new import_client_dynamodb.DynamoDBClient({});
var ORDERS_TABLE = process.env.ORDERS_TABLE;
var ProductReportService = class {
  async getProductReport(params) {
    ...
    if (fromDate && toDate) {
      ...
      toTime = new Date(toDate).getTime() + 864e5;
    } else {
      const days = this.getDays(range || "7d");
      fromTime = now - days * 864e5;
    }
    ...
    const data = await client.send(
      new import_client_dynamodb.ScanCommand({ TableName: ORDERS_TABLE })
    );
    ...
    const highDemand = byQuantity.filter((p) => !revenueLeaderIds.has(p.productId)).slice(0, 5);
    const highDemandIds = new Set(highDemand.map((p) => p.productId));
    const underPerforming = products.filter(
      (p) => p.revenue < 500 && p.quantity <= 2 && !revenueLeaderIds.has(p.productId) && !highDemandIds.has(p.productId)
    );
    const insights = [];
    const totalRevenue = products.reduce((sum, p) => sum + p.revenue, 0);
    const top2Revenue = revenueLeaders.slice(0, 2).reduce((sum, p) => sum + p.revenue, 0);
    ...
    if (revenueLeaders.length > 0 && revenueLeaders[0].revenue > totalRevenue * 0.5) {
      insights.push(
...
*/
//# sourceMappingURL=getProductReport.js.map
