...
// src/handlers/adminApplyChitBalance.ts
var adminApplyChitBalance_exports = {};
__export(adminApplyChitBalance_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(adminApplyChitBalance_exports);
  ...
  async updateOrder(input) {
    ...
    const res = await ddb.send(
      new import_lib_dynamodb2.UpdateCommand({
        ...
        ConditionExpression: "#status <> :cancelled AND #status <> :dispatched",
  ...
  async updateOrderAddress(input) {
    ...
    const res = await ddb.send(
      new import_lib_dynamodb2.UpdateCommand({
        ...
        UpdateExpression: "SET address = :address, modifiedAt = :now, modifiedBy = :by",
        ConditionExpression: "#status <> :cancelled AND #status <> :dispatched",
...
// src/services/adminUpdateOrder.service.ts
var STATUS_ORDER = [
  "ORDER_PLACED",
  "ORDER_CONFIRMED",
  "PAYMENT_CONFIRMED",
...
var AdminUpdateOrderService = class {
  constructor(repo = new AdminUpdateOrderRepository(), orderRepo = new OrderRepository()) {
    this.repo = repo;
    this.orderRepo = orderRepo;
  }
  async updateOrder(input) {
    if (input.status === void 0 && input.adminComment === void 0) {
      throw {
        statusCode: 400,
    ...
    if (input.status === "PAYMENT_CONFIRMED" && !input.paymentAccountId?.trim()) {
      throw {
        statusCode: 400,
  ...
  async updateOrderAddress(input) {
    ...
    const pincode = address.pincode.replace(/\D/g, "").trim();
    if (pincode.length !== 6) {
      throw {
        statusCode: 400,
  ...
  try {
    ...
    if (!Number.isFinite(chitAmount) || chitAmount <= 0) {
      return {
        statusCode: 400,
        body: JSON.stringify({
          message: "Valid chitAmount is required"
        })
      };
    }
...
*/
//# sourceMappingURL=adminApplyChitBalance.js.map
