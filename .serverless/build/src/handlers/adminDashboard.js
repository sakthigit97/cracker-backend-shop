...
// src/handlers/adminDashboard.ts
  ...
  async getTodayOrderCount() {
    ...
    const res = await ddb.send(
      new import_lib_dynamodb2.QueryCommand({
        ...
        KeyConditionExpression: "#meta = :meta AND createdAt >= :start",
      ...
      ALL_STATUSES.map(async (status) => {
        const res = await ddb.send(
          new import_lib_dynamodb2.QueryCommand({
            ...
            KeyConditionExpression: "#status = :status AND #meta = :meta",
            ExpressionAttributeNames: {
              "#status": "status",
              "#meta": "meta"
            },
  ...
  async getRecentOrders(limit) {
    const res = await ddb.send(
      new import_lib_dynamodb2.QueryCommand({
        ...
        Limit: limit,
        ProjectionExpression: "orderId, #status, totalAmount, createdAt"
      })
    );
    return res.Items ?? [];
  }
