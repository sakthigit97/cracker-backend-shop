...
// src/repo/adminGetComboPackages.repo.ts
...
var AdminGetComboPackagesRepository = class {
...
// src/services/adminGetComboPackages.service.ts
var AdminGetComboPackagesService = class {
  constructor(comboRepo = new AdminGetComboPackagesRepository(), configRepo = new AdminConfigRepo()) {
    this.comboRepo = comboRepo;
    this.configRepo = configRepo;
  }
  async listComboPackages() {
    const [comboProducts, allProducts, config] = await Promise.all([
    ...
    const combos = comboProducts.map((comboProduct) => {
      const normalizeComboName = (value) => String(value ?? "").trim().toLowerCase().replace(/\s+/g, " ");
      const packageTag = packageTags.find(
        (tag) => tag?.productId === comboProduct.productId
      ) || packageTags.find(
        (tag) => normalizeComboName(tag?.name) === normalizeComboName(comboProduct.name)
      );
      const comboId = packageTag?.id || null;
      const relatedProducts = comboId ? allProducts.filter(
        (product) => product.isComboPackage !== true && Array.isArray(product.packageTagIds) && product.packageTagIds.includes(comboId)
      ) : [];
      ...
      return {
        ...
        imageUrl: packageTag?.imageUrl || comboProduct.imageUrls?.[0] || "",
        ...
        createdAt: comboProduct.createdAt || null
      };
    });
    combos.sort((a, b) => {
